...
The scatter in Figure 2 is informative. The eleven mesophilic and psychrophilic organisms (OGT 15–37°C) cluster in a relatively narrow Z range without a discernible trend, while the two thermophiles — *Picrophilus torridus* (OGT 60°C) and *Thermus thermophilus* (OGT 70°C) — sit at marginally elevated Z values. From a chemical engineering perspective, this is analogous to a process calibration curve whose apparent slope is determined entirely by the two extreme calibration standards; remove those standards and the curve is flat. The positive ρ of +0.21 must therefore be interpreted cautiously.

The convergence across all three Z metrics in Table 1 (ρ = +0.15 to +0.21) is reassuring in one respect: the result is not a peculiarity of a single geometric formulation. z_mean, z_max, and z_frac each quantify a different facet of surface charge organisation — mean patch score, maximum patch score, and the fraction of patches with any positive Z value — yet all yield the same qualitative trend and the same non-significance. This convergence confirms that the modest positive signal is real at the descriptive level; it simply lacks statistical support at N = 13 organisms.

Table 2 shows that the ρ estimate is insensitive to how strictly one filters organisms by per-organism protein count. Raising MIN_PER_ORG from 30 to 100 leaves ρ fixed at +0.212 because all 13 organisms exceed 100 scored proteins. At MIN_PER_ORG = 200, the *Human body fluids* dataset (N = 100) drops out and ρ rises slightly to +0.297 — a small-sample artefact rather than a meaningful change — while permutation significance remains absent. The result is therefore stable with respect to this operational choice.
...
The LOO results deliver one of the clearest diagnostics in this analysis. Figure 3 shows that removing any of the eleven organisms with OGT ≤ 37°C leaves the correlation essentially unchanged (ρ = +0.175 to +0.329), while removing either thermophile collapses it to exactly zero. A chemical engineer would recognise this immediately: the apparent cross-species trend is structurally equivalent to a two-point extrapolation, where the slope is defined entirely by the two most extreme data points and the remaining eleven organisms provide no leverage over the trend. This is not a signal of a robust physical relationship — it is an artefact of dataset composition.

Table 3 contains a further observation worth noting. Dropping *Escherichia coli* (OGT 37°C) reduces ρ from +0.212 to +0.081 — a 62% decrease from removing a single mesophile. This implies that the *E. coli* proteome's mean Z is anomalously low within the 37°C cluster, artificially steepening the observed trend when it is included. Conversely, dropping *Homo sapiens* produces the highest LOO ρ of +0.329, meaning that the large human dataset (N = 10,695 proteins) pulls the 37°C mean Z upward and partially suppresses the apparent thermophile-driven slope. These organism-to-organism Z variations within a single OGT band further undermine confidence in the cross-species trend and highlight how sensitive ρ is to individual organism characteristics rather than temperature adaptation per se.
...
Figure 4 tells a strikingly different story from the organism-level analysis. Eleven of 13 organisms display a positive Z–Tm relationship, and the pooled signal at ρ = +0.079 is extraordinarily well-supported statistically (p = 1.26×10⁻⁴⁸ across N = 34,231 proteins). A chemical engineer familiar with structure–property relationships in polymer chemistry would recognise this pattern: a structural descriptor correlates with a property within chemically homologous series, even when across-family comparisons are noisy. The within-organism analysis effectively holds phylogenetic background constant by design, which is precisely why it reveals a signal that the cross-species comparison cannot resolve.

Table 4 reveals systematic variation in the per-organism coupling strength that is itself interpretable in light of cellular biology. Bacterial proteomes show the strongest effects: *B. subtilis* (ρ = +0.149, p = 3.88×10⁻⁸) and *E. coli* (ρ = +0.109, p = 1.14×10⁻⁶) are the two highest entries in the table. Bacterial proteins tend to be compact and structurally simple, lack glycosylation, and depend primarily on intrinsic fold stability rather than post-translational scaffolding — conditions under which surface charge organisation is likely to carry a disproportionate fraction of the total stability burden. Moving to eukaryotes, vertebrate proteins (*H. sapiens*, *M. musculus*, *D. rerio*: ρ = 0.078–0.086) and the plant proteome (*Arabidopsis*: ρ = +0.098) occupy a middle range, while *Drosophila melanogaster* (ρ = −0.026, p = 0.31) is the sole outlier with a non-significant negative trend — an anomaly that may reflect the unique thermal biology of insects or the specific composition of the TPP dataset.

Two extremophile organisms stand out at the bottom of the table. The psychrophile *Oleispira antarctica* (ρ = +0.038, p = 0.17) shows no significant Z–Tm coupling. Cold-adapted proteins evolved for conformational flexibility at low temperature through mechanisms such as reduced hydrophobic core packing and increased surface entropy — strategies that are mechanistically distinct from charge-based stabilisation and may render Z largely uninformative for this organism. The acidothermophile *Picrophilus torridus* (ρ = +0.031, p = 0.39) similarly shows a negligible relationship. In *Picrophilus*'s in vivo environment (external pH ~0.7, cytoplasmic pH ~4.6), a large fraction of surface glutamate and aspartate residues are protonated and effectively charge-neutral, fundamentally altering the electrostatic landscape relative to what AlphaFold2 coordinates at standard ionisation state would suggest. The Z score is therefore a poor proxy for true surface charge organisation in this extreme acidophile.
...
The bar chart in Figure 5 places the incremental contribution of Z in quantitative perspective. The absolute R² values are strikingly small: composition features alone explain 0.86% of Tm variance, and adding Z brings this to 1.24%. This is not surprising. Protein thermal stability is a multifactorial property shaped by hydrophobic core packing, secondary structure propensity, disulfide bonding, metal coordination, buried salt bridge networks, loop entropy, and cofactor binding. No single surface descriptor should be expected to dominate this landscape, and a focused metric like Z was never intended as a comprehensive Tm predictor.

Within this context, the ΔR² = +0.0038 (F-test p = 3.67×10⁻³⁰) should be read as positive evidence that Z captures a real, composition-independent signal — not as a claim of practical predictive sufficiency. The semi-standardised coefficient of +0.688 °C/σ_Z can be given a concrete engineering interpretation: a protein redesigned to increase its surface charge mixing by one standard deviation — a substantial but potentially achievable structural change through strategic charge residue placement — would be expected to have a melting temperature approximately 0.7°C higher, all other structural factors held constant. In industrial enzyme engineering, where incremental stability improvements can meaningfully extend operational half-life at process temperatures, this is a modest but non-negligible design handle, particularly because it operates through a mechanism orthogonal to the more commonly targeted strategies of core repacking or disulfide introduction.
...
Figure 6 addresses a natural question that follows from the within-organism analysis: does the strength of the Z–Tm coupling itself scale with OGT? If zwitterionic surface organisation were a temperature-tuned property — progressively optimised in warmer organisms — one would expect the per-organism ρ values to trend upward along the x-axis. They do not (ρ = +0.117, permutation p = 0.71). The coupling strength does not increase in thermophiles relative to mesophiles, nor does it decrease in psychrophiles. The Z–Tm relationship, where present, appears to be a universal intra-proteome feature rather than a temperature-specific adaptation.

The domain-of-life stratification visible in Figure 6 is the key structural observation in this panel. The four bacterial organisms (blue points) cluster in the upper portion of the ρ axis, clearly separated from the eukaryotic cloud (green points) which spans a wider and lower range. The single archaeal organism (orange) sits near the bottom. This vertical separation is orthogonal to OGT: it is a domain-level, not a temperature-level, phenomenon. From a chemical engineering standpoint, this implies that the predictive value of Z as a stability descriptor is domain-dependent. It is most informative for bacterial targets — precisely the class of proteins most commonly engineered for industrial applications — and progressively less informative for eukaryotes, where additional stabilisation mechanisms dilute the fractional contribution of surface electrostatics. Protein engineers working on eukaryotic targets should therefore treat Z as one complementary signal among several, rather than a primary predictor.
...
**Plain-language summary.** Think of a protein surface as a mosaic of positively and negatively charged tiles. The zwitterionic surface score Z measures how evenly those tiles are mixed: a high-Z protein has its positive and negative charges interspersed like a checkerboard, while a low-Z protein has them clustered in separate positive and negative patches. Our results yield two separable conclusions. First, within any individual organism's proteome, the proteins that are hardest to thermally denature tend to display more checkerboard-like surfaces — a real and statistically robust relationship (pooled ρ ≈ +0.08 across 34,000 proteins), corresponding to roughly 0.7°C higher melting temperature per standard deviation of Z, and this holds even after accounting for how many charged residues the protein carries in total. Second, when comparing entire proteomes across organisms living at temperatures ranging from 15°C to 70°C, the ones adapted to higher growth temperatures do not show more checkerboard-like surfaces once we factor out that thermophilic proteomes simply carry more charged residues overall. In practical engineering terms: improving the spatial mixing of surface charges is a real, measurable, and composition-independent lever for modestly improving an individual protein's thermal stability, particularly for bacterial targets. However, it is not the evolutionary strategy that thermophilic organisms have primarily deployed for temperature adaptation — they appear to have increased the total charge density on their protein surfaces rather than reorganising how those charges are spatially arranged. For protein engineers, this means Z provides a directional design handle that is complementary to, and mechanistically distinct from, simply increasing the number of surface acidic and basic residues.
